...
import (
	"bytes"
	poker "github.com/Learn_go_with_tests/WebSockets"
	"io"
	"strings"
	"testing"
	"time"
...
func TestCLI(t *testing.T) {
	...
	t.Run("start game with 3 players and finish game with 'Sonia' as winner", func(t *testing.T) {
		game := &GameSpy{}

		out := &bytes.Buffer{}
		in := userSends("3", "Sonia wins")

		poker.NewCLI(in, out, game).PlayPoker()

		assertMessagesSentToUser(t, out, poker.PlayerPrompt)
		assertGameStartedWith(t, game, 3)
		assertFinishCalledWith(t, game, "Sonia")
	})

	t.Run("start game with 8 players and record 'Jimo' as winner", func(t *testing.T) {
		game := &GameSpy{}

		in := userSends("8", "Jimo wins")

		poker.NewCLI(in, dummyStdOut, game).PlayPoker()

		assertGameStartedWith(t, game, 8)
		assertFinishCalledWith(t, game, "Jimo")
	})
	...
	t.Run("it prints an error when the winner is declared incorrectly", func(t *testing.T) {
		...
		out := &bytes.Buffer{}
		in := userSends("8", "Luke is a killer")
